%% Fixed partition of HOG AND SVM
% runs fixed partition validation on the training set
...
% we want to loop over each value of C used in the SVM model
for c=1:length(cValues)
    cValue = cValues(1,c);

    % fit the SVM model using the HOG Features
    svmModel = fitcsvm(trainingData, trainingLabels, 'KernelFunction', 'rbf', 'BoxConstraint', cValue);
    ...
    for j=1:validationSize
        ...
        [label, score] = predict(svmModel, testImageHOG);
    ...
    % append values to the appropriate vectors
